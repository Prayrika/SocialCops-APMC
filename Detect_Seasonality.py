...
def detect_seasonality(APMC,commodity):
    ...
    data_reqd=data_reqd.sort_index()
    data_reqd['modal_price'].plot(figsize=(12, 5), linewidth=5, fontsize=10)
    plt.title('Modal Price versus Time')
    plt.legend()
    plt.show()
    ...
    fig= plt.figure()
    fig.suptitle('Comparison of Deseasonalised Prices')
    ...
    plt.show()
    fig = plt.figure()
    fig.suptitle('Comparison of Prices After Trend Removal')
    data_reqd['modal_price'].diff().plot(label='First order differencing')
    plt.plot(data_reqd.index, data_reqd['modal_price'])
    fig.autofmt_xdate(rotation=45)
    plt.legend()
    plt.show()
